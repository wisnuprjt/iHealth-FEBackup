...
interface CardListReportHistoryPostTestProps {
  data: PostTest[];
  isLoading: boolean;
}

export default function CardListReportHistoryPostTest({
  data,
  isLoading,
}: CardListReportHistoryPostTestProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((postTest) => (
        <Link
          href={`/dashboard/admin/reports/history/post-test/${postTest.id}`}
          key={postTest.id}
              ...
              <CardHeader className="flex md:flex-row md:items-center md:justify-between">
                <div className="space-y-2">
                  <Badge className="text-white uppercase" variant={"secondary"}>
                    {postTest.sub_module?.module?.type}
                  </Badge>
