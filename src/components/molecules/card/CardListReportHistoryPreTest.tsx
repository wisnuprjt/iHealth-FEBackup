...
interface CardListReportHistoryPreTestProps {
  data: PreTest[];
  isLoading: boolean;
}

export default function CardListReportHistoryPreTest({
  data,
  isLoading,
}: CardListReportHistoryPreTestProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((preTest) => (
        <Link
          href={`/dashboard/admin/reports/history/pre-test/${preTest.id}`}
          key={preTest.id}
